...
export function populateHero(section, data) {
    ...
    // Eyebrow
    if (data.eyebrow) {
        var eyebrowSel = data.eyebrowSelector || '.ds-eyebrow';
        var eyebrow = section.querySelector(eyebrowSel);
        if (eyebrow) {
            var dot = eyebrow.querySelector('.ds-eyebrow-dot') || eyebrow.querySelector('[class*="dot"]');
            eyebrow.textContent = '';
            if (dot) eyebrow.appendChild(dot);
            eyebrow.appendChild(document.createTextNode(' ' + data.eyebrow));
        }
    }
...
export function populateComparisonTable(tableSelector, columns, rows) {
    ...
    tbody.innerHTML = sorted.map(function (r) {
        return '<tr>' +
            '<td>' + (r.feature || '') + '</td>' +
            '<td class="ds-col-icsdc">' + comparisonIcon(r.icsdcStatus || r.icsdc) + '</td>' +
